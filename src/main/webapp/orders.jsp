...
<%
    ...
    List<Order> orders = new OrderDAOImpl().getOrdersByUser(loggedInUser.getUserId());
...
<head>
    ...
    <title>Your Orders — BiteSpeed</title>
    ...
    <main class="orders-main">
                ...
                <table class="orders-table">
                    ...
                    <tbody>
                        <% for (Order o : orders) { 
                            ...
                            try {
                                restName = restaurantDAO.getRestaurant(o.getRestaurantId()).getRestaurantName();
                            } catch (Exception e) {}
                        %>
                            <tr>
                                <td>#BS<%= o.getOrderId() %></td>
                                <td><%= restName %></td>
                                <td><%= o.getOrderDate() %></td>
                                <td>₹<%= o.getTotalAmount() %></td>
                                <td><span class="status-badge <%= o.getOrderStatus().toLowerCase() %>"><%= o.getOrderStatus().toUpperCase() %></span></td>
                                <td><a href="orderDetails.jsp?orderId=<%= o.getOrderId() %>" class="btn-inspect">Inspect Details</a></td>
                            </tr>
                        <% } %>
                    </tbody>
                </table>
        ...
        <% } else { %>
            <div class="empty-orders-view">
                <span>🛸</span>
                <p>No orders registered for this user coordinate.</p>
                <a href="restaurants" class="btn-primary">Browse Resto-Decks</a>
            </div>
        <% } %>
    </main>
</body>
